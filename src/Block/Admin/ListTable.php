...
class CallMeBack_Block_Admin_ListTable {
     ...
     * @var array
     */
    protected $compat_methods = array( 'setPaginationArgs', 'getViews', 'getBulkActions', 'bulkActions',
        'rowActions', 'monthsDropdown', 'viewSwitcher', 'commentsBubble', 'getItemsPerPage', 'pagination',
        'getSortableColumns', 'getColumnInfo', 'getTableClasses', 'displayTablenav', 'extraTablenav',
        'singleRowColumns' );
    ...
    public function __construct( $args = array() ) {
        ...
        if ( $args['ajax'] ) {
            add_action( 'admin_footer', array( $this, 'injectJsVars' ) );
        }
    ...
    public function ajaxUserCan() {
        throw new Exception( 'function WP_List_Table::ajax_user_can() must be over-ridden in a sub-class.' );
    }
    ...
    public function prepareItems() {
        throw new Exception( 'function WP_List_Table::prepare_items() must be over-ridden in a sub-class.' );
    }
     ...
     */
    protected function monthsDropdown( $post_type ) {
        $wpdb = $GLOBALS['wpdb'];
     ...
     */
    protected function commentsBubble( $post_id, $pending_comments ) {
    ...
    public function getColumns() {
        throw new Exception( 'function ListTable::getColumns() must be over-ridden in a sub-class.' );
    }
    ...
    /**
     * Public wrapper for List_Table::getPrimaryColumnName().
     *
     * @since 4.4.0
     * @access public
     *
    ...
    protected function getPrimaryColumnName() {
        ...
        if ( ! isset( $columns[ $default ] ) ) {
            $default = static::getDefaultPrimaryColumnName();
        }
    ...
    protected function getColumnInfo() {
        ...
        if ( isset( $this->_column_headers ) && is_array( $this->_column_headers ) ) {
            ...
            $column_headers = array( array(), array(), array(), $this->getPrimaryColumnName() );
    ...
    protected function singleRowColumns( $item ) {
        $columnInfos = $this->getColumnInfo();
        $columns     = $columnInfos[0];
        $hidden      = $columnInfos[1];
        $primary     = $columnInfos[3];
        ...
        foreach ( $columns as $column_name => $column_display_name ) {
            ...
            if ( 'cb' === $column_name ) {
                ...
                echo "<td $attributes>";
                echo call_user_func( array( $this, 'column' . ucfirst($column_name) ), $item );
